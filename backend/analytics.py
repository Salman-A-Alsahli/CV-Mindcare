...
class Analytics:
    ...
    def detect_trends(
        ...
        logger.info(f"Detected {direction.value} trend for {data_type}: {change_percent:.1f}% change")
        return result
    ...
    def detect_anomalies(
        ...
        data_type: str,
        days: int = 7,
        threshold_stddev: float = ANOMALY_STDDEV_THRESHOLD
    ) -> List[Dict[str, Any]]:
        """
    ...
    def get_chart_data(
        ...
        labels = [
            datetime.fromisoformat(item['timestamp']).strftime(label_format)
    ...
    def _get_raw_data(
        ...
        try:
            ...
            with closing(conn):
                ...
                cursor = conn.execute(
                    query,
                    (data_type, start_time.isoformat(), end_time.isoformat())
                )
