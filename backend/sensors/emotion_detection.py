...
class EmotionDetector(BaseSensor):
    ...
    def __init__(
        ...
        if model_name not in self.AVAILABLE_MODELS:
            logger.warning(
                f"Model '{model_name}' not in available models. " f"Using 'VGG-Face' instead."
            )
    ...
    def initialize(self) -> bool:
        ...
        if not DEEPFACE_AVAILABLE:
            raise SensorUnavailableError(
                "DeepFace library not available. " "Install with: pip install deepface tensorflow"
            )
        ...
        try:
            ...
            if not self.cap.isOpened():
                raise SensorUnavailableError(f"Camera {self.camera_index} not available")

            # Test camera read
            ret, frame = self.cap.read()
            if not ret or frame is None:
                raise SensorUnavailableError(f"Camera {self.camera_index} failed to capture frame")

            logger.info(f"Emotion detector initialized with {self.model_name} model")
            return True
    ...
    def analyze_image(self, image_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if frame is None:
                return {"available": False, "error": f"Failed to load image: {image_path}"}
...
def detect_emotion(camera_index: int = 0, model_name: str = "VGG-Face") -> Dict[str, Any]:
    """
    ...
    if not detector.start():
        return {"available": False, "error": "Failed to start emotion detector"}
...
def analyze_image_emotion(image_path: str, model_name: str = "VGG-Face") -> Dict[str, Any]:
    """
    Analyze emotions from an image file.
