...
class TestDatabaseInitialization:
    ...
    def test_init_db_creates_tables(self, temp_db):
        ...
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='face_detection'")
        assert cursor.fetchone() is not None
        
        # Check sound_analysis table
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='sound_analysis'")
        assert cursor.fetchone() is not None
